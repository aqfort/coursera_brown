...
public:
    ...
    {
        WriteAccess(
            const K &key, Bucket &bucket) : guard(bucket.mutex_), ref_to_value(bucket.data_[key]) {}
    ...
    {
        ReadAccess(
            const K &key, const Bucket &bucket) : guard(bucket.mutex_), ref_to_value(bucket.data_.at(key)) {}
        lock_guard<mutex> guard;
    ...
    {
        auto &bucket = buckets_[(hasher_(key)) % buckets_.size()];
        lock_guard guard_temp(bucket.mutex_);
        return bucket.data_.find(key) != bucket.data_.end();
    }

    MapType BuildOrdinaryMap() const
    {
        MapType result;
        for (auto &[mutex_local, map_local] : buckets_)
        {
            lock_guard<mutex> guard_temp(mutex_local);
            result.insert(map_local.begin(), map_local.end());
        }
        return result;
    }
};
