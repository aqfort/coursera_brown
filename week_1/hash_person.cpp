...
struct Address
{
    ...
    {
        return tie(this->city, this->street, this->building) ==
...
struct Person
{
    ...
    {
        return tie(this->name, this->height, this->weight, this->address) ==
...
struct AddressHasher
{
    ...
    {
        const size_t x = numeric_limits<size_t>::max();
        const hash<string> hasher_string;
        const hash<int> hasher_int;
        return x * x * hasher_string(address.city) +
               x * hasher_string(address.street) +
               hasher_int(address.building);
    }
};
...
struct PersonHasher
{
    size_t operator()(const Person &person) const
    {
        ...
        const hash<string> hasher_string;
        const hash<int> hasher_int;
        const hash<double> hasher_double;
        const AddressHasher hasher_address;
        return x * x * x * hasher_string(person.name) +
               x * x * hasher_int(person.height) +
               x * hasher_double(person.weight) +
               hasher_address(person.address);
    }
};
