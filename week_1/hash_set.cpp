...
public:
    explicit HashSet(
        size_t num_buckets,
        const Hasher &hasher = {}) : data_(num_buckets), hasher_(hasher) {}

    void Add(const Type &value)
    {
        auto &bucket = data_[GetBucketIndex(value)];
        if (!Has(value))
        {
    ...
    {
        data_[GetBucketIndex(value)].remove(value);
    }

    const BucketList &GetBucket(const Type &value) const
    {
        return data_[GetBucketIndex(value)];
    }
